import type { APIEmbed, APIMessageReference } from "@fluxer-selfbot/types";
...
export class MessagePayload {
	...
	setEmbeds(embeds: (APIEmbed | EmbedBuilder)[] | null): this {
		...
		if (embeds.length > EMBEDS_MAX)
			throw new RangeError(`Embeds must be ≤${EMBEDS_MAX}`);
		this.data.embeds = embeds.map((e) =>
	...
	static create(
		contentOrOptions?: string | MessagePayloadData,
	): MessagePayload {
		...
		if (typeof contentOrOptions === "string") {
			...
			if (contentOrOptions.embeds?.length)
				payload.setEmbeds(contentOrOptions.embeds);
			...
			if (contentOrOptions.message_reference)
				payload.setReply(
					contentOrOptions.message_reference as APIMessageReference,
				);
			if (contentOrOptions.tts !== undefined)
				payload.setTTS(contentOrOptions.tts);
			if (contentOrOptions.flags !== undefined)
				payload.setFlags(contentOrOptions.flags);
		}
		return payload;
	}
}
