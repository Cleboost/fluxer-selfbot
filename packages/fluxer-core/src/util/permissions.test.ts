...
describe("computePermissions", () => {
	...
	it("returns full permissions when owner (including high-bit Fluxer perms)", () => {
		...
		expect(hasPermission(perms, BigInt(PermissionFlags.ManageRoles))).toBe(
			true,
		);
		expect(hasPermission(perms, BigInt(PermissionFlags.PinMessages))).toBe(
			true,
		);
		expect(hasPermission(perms, BigInt(PermissionFlags.BypassSlowmode))).toBe(
			true,
		);
	});
