...
import { emitDeprecationWarning, PermissionFlags } from "@fluxer-selfbot/util";
...
export abstract class Channel extends Base {
	...
	static from(
		...
		if (type === ChannelType.GuildText)
			return new TextChannel(client, data as APIChannel);
		if (type === ChannelType.GuildCategory)
			return new CategoryChannel(client, data as APIChannel);
		if (type === ChannelType.GuildVoice)
			return new VoiceChannel(client, data as APIChannel);
		if (
			type === ChannelType.GuildLink ||
			type === ChannelType.GuildLinkExtended
		)
...
export class GuildChannel extends Channel {
	...
	async editPermission(
		...
		const idx = this.permissionOverwrites.findIndex(
			(o) => o.id === overwriteId,
		);
		const entry = {
			id: overwriteId,
	...
	async send(options: MessageSendOptions): Promise<Message> {
		...
		const files = opts.files?.length
			? await resolveMessageFiles(opts.files)
			: undefined;
	...
	async deletePermission(overwriteId: string): Promise<void> {
		await this.client.rest.delete(
			Routes.channelPermission(this.id, overwriteId),
			{ auth: true },
		);
		const idx = this.permissionOverwrites.findIndex(
			(o) => o.id === overwriteId,
	...
	async edit(options: {
		...
		const data = await this.client.rest.patch<APIChannel>(
		...
		if ("rate_limit_per_user" in data)
			self.rateLimitPerUser = data.rate_limit_per_user ?? 0;
		if ("bitrate" in self && "bitrate" in data)
	...
	async delete(options?: { silent?: boolean }): Promise<void> {
		const url =
			Routes.channel(this.id) + (options?.silent ? "?silent=true" : "");
...
export class DMChannel extends Channel {
	...
	constructor(client: Client, data: APIChannelPartial & Partial<APIChannel>) {
		...
		this.recipients = ((data as APIChannel).recipients ?? []).map(
			(u: APIUser) => client.getOrCreateUser(u),
		);
	...
	async addRecipient(userId: string): Promise<void> {
		await this.client.rest.put(Routes.channelRecipient(this.id, userId), {
			auth: true,
		});
		const user =
			this.client.users.get(userId) ?? (await this.client.users.fetch(userId));
	...
	async removeRecipient(
		...
		const url =
			Routes.channelRecipient(this.id, userId) +
