...
export class Message extends Base {
	...
	static async _createMessageBody(
		...
		const files = content.files?.length
			? await resolveMessageFiles(content.files)
			: undefined;
	...
	async pin(): Promise<void> {
		await this.client.rest.put(
			Routes.channelPinMessage(this.channelId, this.id),
		);
		this.pinned = true;
	}
	...
	async removeAllReactions(): Promise<void> {
		await this.client.rest.delete(
			Routes.channelMessageReactions(this.channelId, this.id),
		);
	}
	...
	async removeReactionEmoji(
		...
		const emojiStr = await this.resolveEmojiForReaction(emoji);
		await this.client.rest.delete(
			Routes.channelMessageReaction(this.channelId, this.id, emojiStr),
		);
	}
	...
	async fetchReactionUsers(
		...
		const data = await this.client.rest.get<
			{ users?: APIUserPartial[] } | APIUserPartial[]
		>(route);
