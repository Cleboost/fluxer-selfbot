...
import { Channel, type GuildChannel } from "./Channel.js";
...
export class Guild extends Base {
	...
	constructor(
		client: Client,
		data: APIGuild & { roles?: APIRole[]; ownerId?: string },
	) {
		super();
		this.client = client;
		this.id = data.id;
	...
	async removeRoleFromMember(userId: string, roleId: string): Promise<void> {
		await this.client.rest.delete(
			Routes.guildMemberRole(this.id, userId, roleId),
		);
	}
	...
	async createRole(
		...
		if (options.unicode_emoji !== undefined)
			body.unicode_emoji = options.unicode_emoji;
		if (options.position !== undefined) body.position = options.position;
		if (options.hoist_position !== undefined)
			body.hoist_position = options.hoist_position;
	...
	async fetchRole(roleId: string): Promise<Role> {
		try {
			const data = await this.client.rest.get<APIRole>(
	...
	async fetchBans(): Promise<GuildBan[]> {
		...
		return list.map(
			(b) => new GuildBan(this.client, { ...b, guild_id: this.id }, this.id),
		);
	}
	...
	async fetchVanityURL(): Promise<APIVanityURL> {
		return this.client.rest.get<APIVanityURL>(Routes.guildVanityUrl(this.id), {
			auth: true,
		});
	}
	...
	async setRolePositions(
		...
		const data = await this.client.rest.patch<
			APIRole[] | Record<string, APIRole>
		>(Routes.guildRoles(this.id), { body: updates, auth: true });
		const list = Array.isArray(data) ? data : Object.values(data ?? {});
		for (const r of list) {
	...
	async setRoleHoistPositions(
		...
		const data = await this.client.rest.patch<
			APIRole[] | Record<string, APIRole>
		>(Routes.guildRolesHoistPositions(this.id), { body: updates, auth: true });
		const list = Array.isArray(data) ? data : Object.values(data ?? {});
		for (const r of list) {
			this.roles.set(r.id, new Role(this.client, r, this.id));
		}
	...
	async resetRoleHoistPositions(): Promise<APIRole[]> {
		const data = await this.client.rest.delete<
			APIRole[] | Record<string, APIRole>
		>(Routes.guildRolesHoistPositions(this.id), { auth: true });
		const list = Array.isArray(data) ? data : Object.values(data ?? {});
		for (const r of list) {
			this.roles.set(r.id, new Role(this.client, r, this.id));
		}
	...
	async fetchEmojis(): Promise<GuildEmoji[]> {
		const data = await this.client.rest.get<
			APIEmoji[] | Record<string, APIEmoji>
	...
	async fetchEmoji(emojiId: string): Promise<GuildEmoji> {
		try {
			const data = await this.client.rest.get<APIEmoji>(
				Routes.guildEmoji(this.id, emojiId),
			);
			const emoji = new GuildEmoji(
				this.client,
				{ ...data, guild_id: this.id },
			...
			throw err instanceof FluxerError
				? err
				: new FluxerError("Failed to fetch guild emoji", {
						cause: err as Error,
					});
		}
	}
	...
	async createEmojisBulk(
		...
		return list.map(
			(e) => new GuildEmoji(this.client, { ...e, guild_id: this.id }, this.id),
		);
	}
	...
	async createStickersBulk(
		...
		return list.map(
			(s) =>
				new GuildSticker(this.client, { ...s, guild_id: this.id }, this.id),
		);
	}
}
