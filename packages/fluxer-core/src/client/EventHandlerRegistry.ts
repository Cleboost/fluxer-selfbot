...
import { Channel, type GuildChannel } from "../structures/Channel";
...
handlers.set("MESSAGE_REACTION_REMOVE_ALL", async (client, d) => {
	client.emit(
		...
		d as GatewayMessageReactionRemoveAllDispatchData,
	);
});
...
handlers.set("GUILD_CREATE", async (client, d) => {
	...
	const g = d as APIGuild & {
		...
		members?: Array<
			APIGuildMember & { user: { id: string }; guild_id?: string }
		>;
	};
...
handlers.set("VOICE_STATE_UPDATE", async (client, d) => {
	client.emit(
		...
		d as GatewayVoiceStateUpdateDispatchData,
	);
});
...
handlers.set("VOICE_SERVER_UPDATE", async (client, d) => {
	client.emit(
		...
		d as GatewayVoiceServerUpdateDispatchData,
	);
});
...
handlers.set("MESSAGE_DELETE_BULK", async (client, d) => {
	client.emit(
		...
		d as GatewayMessageDeleteBulkDispatchData,
	);
});
