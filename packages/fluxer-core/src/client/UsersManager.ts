...
export class UsersManager extends Collection<string, User> {
	...
	async fetch(userId: string): Promise<User> {
		const data = await this.client.rest.get<APIUserPartial>(
			Routes.user(userId),
		);
	...
	async fetchWithProfile(
		...
		const [userData, globalProfileData, serverProfileData, memberData] =
			await Promise.all([
				this.client.rest.get<APIUserPartial>(Routes.user(userId)),
				this.client.rest
					.get(Routes.userProfile(userId))
				...
				guildId
					? this.client.rest
							.get<APIGuildMember & { user: { id: string } }>(
