...
export class VoiceConnection extends EventEmitter {
	...
	async connect(
		...
		if (raw.includes("?")) {
			...
			wsUrl = /^wss?:\/\//i.test(raw)
				? raw
				: raw.replace(/^https?:\/\//i, "wss://");
			if (!/^wss?:\/\//i.test(wsUrl)) wsUrl = `wss://${wsUrl}`;
		} else {
			const normalized =
				raw.replace(/^(wss|ws|https?):\/\//i, "").replace(/^\/+/, "") || raw;
		...
		return new Promise((resolve, reject) => {
			...
			ws.on("message", (data: unknown) =>
				onMessage(data as Buffer | ArrayBuffer),
			);
			ws.once("close", () => {
				cleanup();
	...
	private async getWebSocketConstructor(): Promise<
		new (
			url: string,
		) => VoiceWebSocket
	> {
	...
	playOpus(stream: NodeJS.ReadableStream): void {
		...
		this.pacingInterval = setInterval(() => {
			...
			if (packet && this.secretKey && this.udpSocket)
				this.sendAudioFrame(packet);
		...
		stream.on("data", (chunk: Buffer) => {
			...
			if (Buffer.isBuffer(chunk) && chunk.length > 0)
				this.audioPacketQueue.push(chunk);
		});
