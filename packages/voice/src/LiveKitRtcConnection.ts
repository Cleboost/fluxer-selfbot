	...
	esds?: {
		esd?: {
			findDescriptor: (tag: number) => {
				findDescriptor: (tag: number) => { data?: Uint8Array };
			};
		};
	};
...
export type LiveKitRtcConnectionEvents = VoiceConnectionEvents & {
	...
	requestVoiceStateSync: [
		payload: { self_stream?: boolean; self_video?: boolean },
	];
...
export class LiveKitRtcConnection extends EventEmitter {
	...
	private readonly receiveSubscriptions = new Map<
		string,
	...
	isSameServer(endpoint: string | null, token: string): boolean {
		...
		return (
			ep === (this.lastServerEndpoint ?? "") &&
			token === (this.lastServerToken ?? "")
		);
	}
	...
	private subscribeParticipantTrack(
		...
		if (
			...
			!this.requestedSubscriptions.has(participantId)
		)
			return;
	...
	playOpus(_stream: NodeJS.ReadableStream): void {
		this.emit(
			"error",
			new Error(
				"LiveKit: playOpus not supported; use play(url) with a WebM/Opus URL",
			),
		);
	}
	...
	async connect(
		...
		try {
			...
			room.on(RoomEvent.ActiveSpeakersChanged, (speakers) => {
				...
				this.activeSpeakers.clear();
				for (const participantId of next)
					this.activeSpeakers.add(participantId);
			});
	...
	async playVideo(
		...
		let useFFmpeg =
			options?.useFFmpeg ?? process.env.FLUXER_VIDEO_FFMPEG === "1";
		...
		if (
			useFFmpeg &&
			(urlOrBuffer instanceof ArrayBuffer || urlOrBuffer instanceof Uint8Array)
		) {
			this.emit(
				"error",
				new Error("useFFmpeg requires a URL; buffer/ArrayBuffer not supported"),
			);
			return;
		}
		...
		file.onReady = (info: {
			...
			const audioTrackInfo = tracks.find(
				(t: { type: string; codec: string }) =>
					t.type === "audio" && t.codec.startsWith("mp4a"),
			);
			...
			if (
				videoTrack.codec.startsWith("avc1") ||
				videoTrack.codec.startsWith("avc3")
			) {
				...
				const trak = isoFile.moov?.traks?.find(
					(t: Trak) => t.tkhd.track_id === videoTrack.id,
				);
			...
			const decoder = new VideoDecoder({
				output: async (frame: WebCodecsVideoFrame) => {
					...
					try {
						...
						const frameTimestampUs =
							(frame as { timestamp?: number }).timestamp ?? 0;
						const frameTimeMs = frameTimestampUs / 1000;

						const copyOptions =
							frame.format !== "I420" ? { format: "I420" as const } : undefined;
						const size = frame.allocationSize(copyOptions);
						const buffer = new Uint8Array(size);
						await frame.copyTo(buffer, copyOptions);
						frame.close();

						const expectedI420Size = Math.ceil(
							(codedWidth * codedHeight * 3) / 2,
						);
							...
							if (tid === videoTrack.id) {
								try {
									for (const sample of samp) {
										const isKeyFrame =
											sample.is_sync ??
											(sample as { is_rap?: boolean }).is_rap ??
											sample.dts === 0;
										const chunk = new EncodedVideoChunk({
											type: isKeyFrame ? "key" : "delta",
											timestamp: Math.round(
												(sample.dts / sample.timescale) * 1_000_000,
											),
											duration: Math.round(
			...
			Promise.all([publishVideo, publishAudio])
				.then(async () => {
					...
					this.currentVideoStream = file as unknown as {
						destroy?: () => void;
						stop?: () => void;
					};
					file.setExtractionOptions(videoTrack.id, null, { nbSamples: 16 });
					pacingInterval = setInterval(() => {
						if (!this._playingVideo || !source || playbackStartMs === null)
							return;
						...
						if (frameQueue.length > 10) {
							while (
								frameQueue.length > 1 &&
								frameQueue[1]?.timestampMs <= elapsed
							) {
								frameQueue.shift();
							}
						}
						...
						if (
							frameQueue.length > 0 &&
							frameQueue[0]?.timestampMs <= elapsed
						) {
							...
							try {
								...
								if (VOICE_DEBUG)
									this.audioDebug("captureFrame error", {
					...
					if (videoUrl && audioSource && audioTrack) {
						const runAudioFfmpeg = async () => {
							...
							const processOneOpusFrame = async (frame: Uint8Array) => {
								if (frame.length < 2 || !audioSource || !this._playingVideo)
									return;
								try {
									...
									const newBuffer = new Int16Array(
										sampleBuffer.length + int16.length,
									);
									...
									while (
										...
										const outSamples = applyVolumeToInt16(
											rawSamples,
											this._volume,
										);
										...
										if (audioSource.queuedDuration > 500)
							...
							demuxer.on("data", (chunk: Buffer) => {
								if (!this._playingVideo) return;
								opusBuffer = new Uint8Array(
									concatUint8Arrays(opusBuffer, new Uint8Array(chunk)),
								);
								while (opusBuffer.length > 0) {
									const parsed = parseOpusPacketBoundaries(opusBuffer);
									if (!parsed) break;
									opusBuffer = new Uint8Array(
										opusBuffer.subarray(parsed.consumed),
									);
	...
	private async playVideoFFmpeg(
		...
		const pushFramesFromBuffer = () => {
			...
			if (frameBufferBytes > frameSize * MAX_QUEUED_FRAMES) {
				...
				const framesToDrop = Math.floor(
					(frameBufferBytes - frameSize * 2) / frameSize,
				);
		...
		const runFFmpeg = async () => {
			...
			if (stderr) {
				stderr.on("data", (data: Buffer) => {
					...
					if (line && VOICE_DEBUG)
						this.audioDebug("ffmpeg stderr", { line: line.slice(0, 200) });
				});
			}
		...
		runFFmpeg().catch((e) =>
			this.audioDebug("ffmpeg error", { error: String(e) }),
		);
	}
	...
	async play(urlOrStream: string | NodeJS.ReadableStream): Promise<void> {
		...
		demuxer.on("data", (chunk: Buffer) => {
			...
			drainOpusQueue().catch((e) =>
				this.audioDebug("drainOpusQueue error", { error: String(e) }),
			);
		});
