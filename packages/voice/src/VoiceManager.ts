...
import { type Client, Events, type VoiceChannel } from "@fluxer-selfbot/core";
import {
	GatewayOpcodes,
...
export class VoiceManager extends EventEmitter {
	...
	private readonly connections = new Collection<
		string,
		VoiceConnection | LiveKitRtcConnection
	>();
	...
	subscribeChannelParticipants(
		...
		const participants = this.listParticipantsInChannel(
			guildId,
			channelId,
		).filter((participantId) => participantId !== this.client.user?.id);
	...
	private handleVoiceServerUpdate(
		...
		if (
			conn instanceof LiveKitRtcConnection &&
			conn.isSameServer(data.endpoint, data.token)
		) {
			return;
		}
	...
	private registerConnection(
		...
		conn.on(
			...
			(p: { self_stream?: boolean; self_video?: boolean }) => {
	...
	private async uploadStreamPreview(
		...
		this.client.emit?.(
			"debug",
			`[VoiceManager] Uploaded stream preview for ${streamKey}`,
		);
	}
	...
	private tryCompletePending(
		...
		const useLiveKit = isLiveKitEndpoint(
			pending.server.endpoint,
		...
		this.storeConnectionId(
			channelId,
			pending.server.connection_id ??
				(state as { connection_id?: string }).connection_id,
		);
	...
	async join(
		...
		if (existing) {
			const isReusable =
				existing instanceof LiveKitRtcConnection
	...
	leave(guildId: string): void {
		...
		for (const [cid, c] of this.connections) {
			if (c?.channel?.guildId === guildId)
				toLeave.push({ channelId: cid, conn: c });
		}
