...
describe("resolvePermissionsToBitfield", () => {
	...
	it("resolves number to string", () => {
		expect(resolvePermissionsToBitfield(2048)).toBe("2048");
		expect(resolvePermissionsToBitfield(PermissionFlags.SendMessages)).toBe(
			"2048",
		);
	});
	...
	it("resolves array of permissions with OR", () => {
		const result = resolvePermissionsToBitfield([
			"SendMessages",
			"ViewChannel",
		]);
	...
	it("throws for invalid permission string", () => {
		expect(() => resolvePermissionsToBitfield("InvalidPermission")).toThrow(
			RangeError,
		);
	});
});
