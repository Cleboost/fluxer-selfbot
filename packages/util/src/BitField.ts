...
export class BitField<S extends string> {
	...
	static resolve<S extends string>(bit: BitFieldResolvable<S>): number {
		const Flags = BitField.Flags as Record<string, number>;
		if (typeof bit === "number" && bit >= 0) return bit;
		if (bit instanceof BitField) return bit.bitfield;
		if (Array.isArray(bit))
			return bit.map((b) => BitField.resolve(b)).reduce((a, b) => a | b, 0);
	...
	serialize(): Record<S, boolean> {
		const Flags = (this.constructor as typeof BitField).Flags as Record<
			S,
			number
		>;
