...
describe("FluxerAPIError", () => {
	...
	it("isRetryable returns true for 5xx", () => {
		expect(
			new FluxerAPIError({ message: "Server error", code: "INTERNAL" }, 500)
				.isRetryable,
		).toBe(true);
		expect(
			new FluxerAPIError({ message: "Bad gateway", code: "BAD_GATEWAY" }, 502)
				.isRetryable,
		).toBe(true);
		expect(
			new FluxerAPIError({ message: "Unavailable", code: "UNAVAILABLE" }, 503)
				.isRetryable,
		).toBe(true);
	});
	...
	it("isRetryable returns false for 4xx (except 429)", () => {
		expect(
			new FluxerAPIError({ message: "Not found", code: "NOT_FOUND" }, 404)
				.isRetryable,
		).toBe(false);
		expect(
			new FluxerAPIError({ message: "Forbidden", code: "FORBIDDEN" }, 403)
				.isRetryable,
		).toBe(false);
		expect(
			new FluxerAPIError({ message: "Bad request", code: "BAD_REQUEST" }, 400)
				.isRetryable,
		).toBe(false);
	});
});
