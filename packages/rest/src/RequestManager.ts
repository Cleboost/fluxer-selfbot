	...
	async request<T>(
		...
		for (let attempt = 0; attempt <= this.options.retries; attempt++) {
			...
			try {
				...
				if (err instanceof FluxerAPIError || err instanceof HTTPError)
					throw err;
