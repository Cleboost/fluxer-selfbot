...
function _getReturnsFromJSDoc(comment: string): string | undefined {
	...
	if (tag && (tag as any).type === "ReturnTag") {
		return ((tag as any).description ?? "").trim();
	}
	return undefined;
}
...
function getExamplesFromJSDoc(comment: string): string[] {
	...
	return parsed.tags
		...
		.map((t) => ((t as any).description ?? "").trim())
		.filter(Boolean);
}

export function getDeprecatedFromJSDoc(
	comment: string,
	...
	const desc = (tag as any).description?.trim();
	return desc || true;
}

/** @discordJsCompat or @discordJsCompat https://discord.js.org/... — string = URL, true = no link */
export function getDiscordJsCompatFromJSDoc(
	comment: string,
): boolean | string | undefined {
	const parsed = parseJSDoc(comment);
	if (!parsed?.tags) return undefined;
