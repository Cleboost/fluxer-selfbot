...
export const Routes = {
	...
	channelBulkDelete: (id: Snowflake) =>
		`/channels/${id}/messages/bulk-delete` as const,
	...
	guildDetachedBanner: (guildId: Snowflake) =>
		`/guilds/${guildId}/detached-banner` as const,
	...
	guildEmojisBulk: (guildId: Snowflake) =>
		`/guilds/${guildId}/emojis/bulk` as const,
	guildStickersBulk: (guildId: Snowflake) =>
		`/guilds/${guildId}/stickers/bulk` as const,
	...
	webhookExecute: (id: Snowflake, token: string) =>
		`/webhooks/${id}/${token}` as const,
	...
	userProfile: (id: Snowflake, guildId?: Snowflake): string =>
		guildId
			? `/users/${id}/profile?guild_id=${guildId}`
			: `/users/${id}/profile`,
	...
	// OAuth2 / Bot
	oauth2ApplicationBot: (id: Snowflake) =>
		`/oauth2/applications/${id}/bot` as const,
